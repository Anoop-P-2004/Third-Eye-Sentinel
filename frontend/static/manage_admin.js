...
function confirmDelete(username) {
    if (confirm("Are you sure you want to delete this user?")) {
        ...
        const token = localStorage.getItem('access-token');
        const path="/manage_admin/delete_admin?Authorization="+encodeURIComponent(token)+"&username="+username;
        fetch(path)
...
function makeAdmin(username) {
    const token = localStorage.getItem('access-token');
    const path="/manage_admin/remove_admin?Authorization="+encodeURIComponent(token)+"&username="+username;
    fetch(path)
    .then(response => response.json())
    .then(data => {
        if (!data.flag) {
